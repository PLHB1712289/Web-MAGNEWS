...
router.get("/", async function (req, res, next) {
  ...
  let data = (await getData(webVOV.categoryLifeStyle, pageNumber)).slice(0);
  ...
  if (data.length > 33) {
    if (pageNumber - 1 > 0) {
      listPage.push({
        ...
        link: `/life-style?page=${pageNumber - 1}`,
        isActive: false,
      });
    }
    listPage.push({
      pageNumber: pageNumber,
      link: `/life-style?page=${pageNumber}`,
      isActive: true,
    });
    listPage.push({
      pageNumber: pageNumber + 1,
