...
router.get('/', async function(req, res, next) {
  ...
  const data = await scrapingVOV(webVOV.categoryEntertainment);
  const {newsFeaturePostLarge, newsFeaturePost, newsPost, newsShortContent} = groupingNews(data, "entertainment");

  res.render('categories/viewNews', 
  {newsFeaturePostLarge, newsFeaturePost, newsPost, newsShortContent, 
  categoryNews: "Giải trí", temp});
});
