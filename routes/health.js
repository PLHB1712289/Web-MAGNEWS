...
router.get("/", async function (req, res, next) {
  ...
  let data = (await getData(webVOV.categoryHealth, pageNumber)).slice(0);
  ...
  if (data.length > 33) {
    if (pageNumber - 1 > 0) {
      listPage.push({
        ...
        link: `/health?page=${pageNumber - 1}`,
        isActive: false,
      });
    }
    listPage.push({
      pageNumber: pageNumber,
      link: `/health?page=${pageNumber}`,
      isActive: true,
    });
    listPage.push({
      pageNumber: pageNumber + 1,
